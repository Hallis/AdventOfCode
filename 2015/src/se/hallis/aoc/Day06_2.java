 ...
 *
 */
public class Day06_2 extends AdventOfCodeBase
{
	...
	private int[][] m_ledarr = new int[MAX_X + 1][MAX_Y + 1];
		...
		for (int x = startX; x <= endX; x++) {
			for (int y = startY; y <= endY; y++) {
				if (Operation.On == op)
					...
					int tmp = m_ledarr[x][y] - 1;
					m_ledarr[x][y] = (tmp <= 0) ? 0 : tmp;
				} else
					m_ledarr[x][y] = m_ledarr[x][y] + 2;
			}
		}
	}
